...
LR_D = 0.00006  # 判别器学习率
BETA1 = 0.5
# 调整损失权重：SSIM优先（权重最高），其次是MSE，保持感知和边缘损失
LAMBDA_SSIM = 80.0  # SSIM损失权重（最高）
LAMBDA_MSE = 20.0   # MSE损失权重（次要）
LAMBDA_PERCEPTUAL = 20.0  # 感知损失权重
LAMBDA_EDGE = 8.0  # 边缘损失权重
LAMBDA_GAN = 0.8  # GAN损失权重
LAMBDA_GP = 10.0  # 梯度惩罚权重
EMA_DECAY = 0.999  # EMA衰减系数

# ==========================================
# 修改点 2：数据路径更新为 30 的文件夹
# ==========================================
...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x1 = self.features1(x_3ch)
        y1 = self.features1(y_3ch)

        x2 = self.features2(x_3ch)
        y2 = self.features2(y_3ch)

        x3 = self.features3(x_3ch)
        y3 = self.features3(y_3ch)
...
class SpatialAttentionResidualBlock(nn.Module):
    def __init__(self, in_channels, device):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1).to(device)
        self.bn1 = nn.BatchNorm2d(in_channels).to(device)
        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1).to(device)
        self.bn2 = nn.BatchNorm2d(in_channels).to(device)

        self.spatial_attention = nn.Sequential(
            nn.Conv2d(in_channels, 1, kernel_size=7, padding=3).to(device),
            nn.Sigmoid()
        )

        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels, in_channels // 4, kernel_size=1).to(device),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 4, in_channels, kernel_size=1).to(device),
            nn.Sigmoid()
        )
...
class Generator(nn.Module):
    def __init__(self, device):
        ...
        self.init_conv = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=7, stride=1, padding=3).to(device),
            nn.BatchNorm2d(64).to(device),
            nn.ReLU(inplace=True)
        )

        self.down1 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1).to(device),
            nn.BatchNorm2d(128).to(device),
            nn.ReLU(inplace=True)
        )

        self.down2 = nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1).to(device),
            nn.BatchNorm2d(256).to(device),
            nn.ReLU(inplace=True)
        )

        self.res1 = SpatialAttentionResidualBlock(256, device)
        self.res2 = SpatialAttentionResidualBlock(256, device)
        self.res3 = SpatialAttentionResidualBlock(256, device)
        self.res4 = SpatialAttentionResidualBlock(256, device)
        self.res5 = SpatialAttentionResidualBlock(256, device)
        ...
        self.width_expansion = nn.Sequential(
            ...
            nn.BatchNorm2d(128).to(device),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(128, 128, kernel_size=(1, 4), stride=(1, 2), padding=(0, 1)).to(device),
            nn.BatchNorm2d(128).to(device),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(128, 64, kernel_size=(1, 5), stride=(1, 3), padding=(0, 1)).to(device),
            nn.BatchNorm2d(64).to(device),
            nn.ReLU(inplace=True)
        )

        self.final_conv = nn.Sequential(
            nn.Conv2d(64 + 64, 64, kernel_size=3, stride=1, padding=1).to(device),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1).to(device),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 1, kernel_size=7, stride=1, padding=3).to(device),
            nn.Tanh()
        )
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"保存{dataset_name}集输出"):
            ...
            for i in range(imu_orig.size(0)):
                ...
                axes[0].set_title(f'IMU Input (x={x}, y={y})')
                axes[0].axis('off')

                axes[1].imshow(video_np, cmap='gray')
                axes[1].set_title(f'Real Video: {video_filename}')
                axes[1].axis('off')

                axes[2].imshow(generated_np, cmap='gray')
                axes[2].set_title(f'Generated (MSE={sample_mse:.4f}, SSIM={sample_ssim:.4f})')
                axes[2].axis('off')

                plt.tight_layout()
                compare_save_path = os.path.join(save_dir, f"compare_{video_filename}")
        ...
        with open(os.path.join('metrics', f'{dataset_name}_statistics.txt'), 'w') as f:
            ...
            f.write(f"最小MSE: {np.min(mse_values):.6f}\n")
            f.write(f"最大MSE: {np.max(mse_values):.6f}\n\n")

            f.write(f"平均SSIM: {np.mean(ssim_values):.6f}\n")
            f.write(f"SSIM标准差: {np.std(ssim_values):.6f}\n")
            f.write(f"最小SSIM: {np.min(ssim_values):.6f}\n")
            f.write(f"最大SSIM: {np.max(ssim_values):.6f}\n")
...
def test(epoch, generator, discriminator, ema):
    ...
    if (epoch % 5 == 0):
        print(f"保存第 {epoch} 个epoch的可视化结果...")
        with torch.no_grad():
            ...
            num_samples = min(5, imu_orig.size(0))
            ...
            for i in range(num_samples):
                ...
                axes[i, 0].imshow(imu_np, cmap='gray')
                axes[i, 0].set_title(f'IMU Input (x={x}, y={y})')
                axes[i, 0].axis('off')

                axes[i, 1].imshow(video_np, cmap='gray')
                axes[i, 1].set_title(f'Real Video: {video_filename}')
                axes[i, 1].axis('off')

                axes[i, 2].imshow(generated_np, cmap='gray')
                axes[i, 2].set_title(f'Generated (MSE={sample_mse:.4f}, SSIM={sample_ssim:.4f})')
                axes[i, 2].axis('off')

            plt.tight_layout()
            plt.savefig(f'results/test_epoch_{epoch}.png')
            plt.close()
...
def train():
    ...
    mse_criterion = nn.MSELoss()  # 改用MSE损失
    ...
    for epoch in range(EPOCHS):
        ...
        running_loss_G_ssim = 0.0  # 新增：SSIM损失
        running_loss_G_mse = 0.0   # 新增：MSE损失
        running_loss_G_perceptual = 0.0
        running_loss_G_edge = 0.0
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{EPOCHS}")):
            ...
            if fake_video.shape[2:] != video_norm.shape[2:]:
                fake_video = F.interpolate(fake_video, size=video_norm.shape[2:], mode='bilinear',
                                           align_corners=True)
            ...
            # 计算SSIM损失（1 - SSIM，因为SSIM越大越好）
            ssim_score = ssim(fake_video_orig, video_orig)
            loss_G_ssim = 1 - ssim_score  # SSIM损失：值越小越好
            ...
            loss_G = (LAMBDA_GAN * loss_G_gan +
                      LAMBDA_SSIM * loss_G_ssim +  # SSIM损失优先
                      LAMBDA_MSE * loss_G_mse +     # MSE次要
            ...
            running_loss_G_ssim += loss_G_ssim.item()
            running_loss_G_mse += loss_G_mse.item()
            running_loss_G_perceptual += loss_G_perceptual.item()
            running_loss_G_edge += loss_G_edge.item()

        scheduler_G.step()
        scheduler_D.step()

        # 打印损失
        avg_loss_G = running_loss_G / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_D = running_loss_D / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_G_gan = running_loss_G_gan / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_G_ssim = running_loss_G_ssim / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_G_mse = running_loss_G_mse / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_G_perceptual = running_loss_G_perceptual / len(train_loader) if len(train_loader) > 0 else 0.0
        avg_loss_G_edge = running_loss_G_edge / len(train_loader) if len(train_loader) > 0 else 0.0

        print(f"Epoch {epoch + 1}, "
              f"Loss_G: {avg_loss_G:.4f} (GAN: {avg_loss_G_gan:.4f}, SSIM: {avg_loss_G_ssim:.4f}, MSE: {avg_loss_G_mse:.4f}, "
              f"Perceptual: {avg_loss_G_perceptual:.4f}, Edge: {avg_loss_G_edge:.4f}), "
    ...
    print("训练完成!")
    ema.apply_shadow()
    torch.save(generator.state_dict(), "models/generator_ema_final.pth")
    ema.restore()
...
if __name__ == "__main__":
    print("开始训练...")
    print(f"预期视频尺寸: {VIDEO_SHAPE} (高度, 宽度)")
    train()
    print("所有任务完成!")